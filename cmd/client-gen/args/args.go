...
type Args struct {
	...
	// SingleClusterTypedClientsPackagePath is the Go import-path for the package containing the single-cluster client path if external clients are used.
	SingleClusterTypedClientsPackagePath string

	// SingleClusterApplyConfigPackagePath is the Go import-path for the package containing the single-cluster apply configuration package path if external apply configuration package is used.
	SingleClusterApplyConfigPackagePath string

	// StaticFakesExpansions is a comma separated list of group/version/type to expand static fakes for.
	StaticFakesExpansions []string
}
...
func (args *Args) AddFlags(fs *pflag.FlagSet, inputBase string) {
	...
	fs.StringVar(&args.SingleClusterTypedClientsPackagePath, "single-cluster-typed-client-package-path", "",
	...
	fs.StringVar(&args.SingleClusterApplyConfigPackagePath, "single-cluster-apply-configuration-package-path", "",
		"the Go import-path for the package containing the single-cluster apply configuration package when generating cluster-scoped apply configuration using upstream package.")
	fs.StringSliceVar(&args.StaticFakesExpansions, "static-fakes-expansions", []string{},
		"comma separated list of group/version/type to expand static fakes for")
