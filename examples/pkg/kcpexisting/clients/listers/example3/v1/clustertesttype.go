...
type ClusterTestTypeClusterLister interface {
	...
	Cluster(clusterName logicalcluster.Name) example3v1listers.ClusterTestTypeLister
...
func (s *clusterTestTypeClusterLister) List(selector labels.Selector) (ret []*example3v1.ClusterTestType, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*example3v1.ClusterTestType))
	})
	return ret, err
}

// Cluster scopes the lister to one workspace, allowing users to list and get ClusterTestTypes.
func (s *clusterTestTypeClusterLister) Cluster(clusterName logicalcluster.Name) example3v1listers.ClusterTestTypeLister {
	return &clusterTestTypeLister{indexer: s.indexer, clusterName: clusterName}
}

// clusterTestTypeLister implements the example3v1listers.ClusterTestTypeLister interface.
...
func (s *clusterTestTypeLister) Get(name string) (*example3v1.ClusterTestType, error) {
	...
	if !exists {
		return nil, errors.NewNotFound(example3v1.Resource("clustertesttypes"), name)
	}
