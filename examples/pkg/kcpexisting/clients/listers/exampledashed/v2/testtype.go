...
type TestTypeClusterLister interface {
	...
	List(selector labels.Selector) (ret []*exampledashedv2.TestType, err error)
	// Cluster returns a lister that can list and get TestTypes in one workspace.
	Cluster(clusterName logicalcluster.Name) exampledashedv2listers.TestTypeLister
...
func (s *testTypeClusterLister) List(selector labels.Selector) (ret []*exampledashedv2.TestType, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*exampledashedv2.TestType))
	})
	return ret, err
}

// Cluster scopes the lister to one workspace, allowing users to list and get TestTypes.
func (s *testTypeClusterLister) Cluster(clusterName logicalcluster.Name) exampledashedv2listers.TestTypeLister {
	return &testTypeLister{indexer: s.indexer, clusterName: clusterName}
}

// testTypeLister implements the exampledashedv2listers.TestTypeLister interface.
...
func (s *testTypeLister) List(selector labels.Selector) (ret []*exampledashedv2.TestType, err error) {
	err = kcpcache.ListAllByCluster(s.indexer, s.clusterName, selector, func(i interface{}) {
		ret = append(ret, i.(*exampledashedv2.TestType))
	})
	return ret, err
}

// TestTypes returns an object that can list and get TestTypes in one namespace.
func (s *testTypeLister) TestTypes(namespace string) exampledashedv2listers.TestTypeNamespaceLister {
	return &testTypeNamespaceLister{indexer: s.indexer, clusterName: s.clusterName, namespace: namespace}
}

// testTypeNamespaceLister implements the exampledashedv2listers.TestTypeNamespaceLister interface.
...
func (s *testTypeNamespaceLister) List(selector labels.Selector) (ret []*exampledashedv2.TestType, err error) {
...
func (s *testTypeNamespaceLister) Get(name string) (*exampledashedv2.TestType, error) {
	...
	if !exists {
		return nil, errors.NewNotFound(exampledashedv2.Resource("testtypes"), name)
	}
	return obj.(*exampledashedv2.TestType), nil
}
