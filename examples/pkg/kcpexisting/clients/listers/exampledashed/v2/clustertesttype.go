...
type ClusterTestTypeClusterLister interface {
	...
	List(selector labels.Selector) (ret []*exampledashedv2.ClusterTestType, err error)
	// Cluster returns a lister that can list and get ClusterTestTypes in one workspace.
	Cluster(clusterName logicalcluster.Name) exampledashedv2listers.ClusterTestTypeLister
...
func (s *clusterTestTypeClusterLister) List(selector labels.Selector) (ret []*exampledashedv2.ClusterTestType, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*exampledashedv2.ClusterTestType))
	})
	return ret, err
}

// Cluster scopes the lister to one workspace, allowing users to list and get ClusterTestTypes.
func (s *clusterTestTypeClusterLister) Cluster(clusterName logicalcluster.Name) exampledashedv2listers.ClusterTestTypeLister {
	return &clusterTestTypeLister{indexer: s.indexer, clusterName: clusterName}
}

// clusterTestTypeLister implements the exampledashedv2listers.ClusterTestTypeLister interface.
...
func (s *clusterTestTypeLister) List(selector labels.Selector) (ret []*exampledashedv2.ClusterTestType, err error) {
	err = kcpcache.ListAllByCluster(s.indexer, s.clusterName, selector, func(i interface{}) {
		ret = append(ret, i.(*exampledashedv2.ClusterTestType))
	})
	return ret, err
}

// Get retrieves the ClusterTestType from the indexer for a given workspace and name.
func (s *clusterTestTypeLister) Get(name string) (*exampledashedv2.ClusterTestType, error) {
	...
	if !exists {
		return nil, errors.NewNotFound(exampledashedv2.Resource("clustertesttypes"), name)
	}
	return obj.(*exampledashedv2.ClusterTestType), nil
}
