...
type ClusterTestTypeLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*somegroup2v1.ClusterTestType, error)
...
// List lists all ClusterTestTypes in the indexer for a workspace.
func (s *clusterTestTypeScopedLister) List(selector labels.Selector) (ret []*somegroup2v1.ClusterTestType, err error) {
...
// Get retrieves the ClusterTestType from the indexer for a given workspace and name.
func (s *clusterTestTypeScopedLister) Get(name string) (*somegroup2v1.ClusterTestType, error) {
	key := name
	obj, exists, err := s.indexer.GetByKey(key)
	if err != nil {
		return nil, err
