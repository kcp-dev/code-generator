...
// TestTypeLister helps list example3v1.TestType.
// All objects returned here must be treated as read-only.
type TestTypeClusterLister interface {
	// List lists all example3v1.TestType in the indexer.
	// Objects returned here must be treated as read-only.
	List(selector labels.Selector) (ret []*example3v1.TestType, err error)

	// Cluster returns an object that can list and get example3v1.TestType from the given logical cluster.
...
// Cluster returns an object that can list and get example3v1.TestType.
...
type TestTypeLister interface {
	...
	// Objects returned here must be treated as read-only.
	List(selector labels.Selector) (ret []*example3v1.TestType, err error)
	// TestTypes returns an object that can list and get example3v1.TestType.
...
// TestTypeNamespaceLister helps list and get example3v1.TestType.
// All objects returned here must be treated as read-only.
type TestTypeNamespaceLister interface {
	// List lists all example3v1.TestType in the indexer for a given namespace.
	// Objects returned here must be treated as read-only.
	List(selector labels.Selector) (ret []*example3v1.TestType, err error)
	// Get retrieves the example3v1.TestType from the indexer for a given namespace and name.
	// Objects returned here must be treated as read-only.
