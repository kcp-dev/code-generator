...
// List lists all StorageClasses in the indexer for a workspace.
func (s *storageClassScopedLister) List(selector labels.Selector) (ret []*storagev1.StorageClass, err error) {
...
// Get retrieves the StorageClass from the indexer for a given workspace and name.
func (s *storageClassScopedLister) Get(name string) (*storagev1.StorageClass, error) {
	key := name
	obj, exists, err := s.indexer.GetByKey(key)
	if err != nil {
		return nil, err
