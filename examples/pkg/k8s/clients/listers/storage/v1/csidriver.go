...
// List lists all CSIDrivers in the indexer for a workspace.
func (s *cSIDriverScopedLister) List(selector labels.Selector) (ret []*storagev1.CSIDriver, err error) {
...
// Get retrieves the CSIDriver from the indexer for a given workspace and name.
func (s *cSIDriverScopedLister) Get(name string) (*storagev1.CSIDriver, error) {
	key := name
	obj, exists, err := s.indexer.GetByKey(key)
	if err != nil {
		return nil, err
