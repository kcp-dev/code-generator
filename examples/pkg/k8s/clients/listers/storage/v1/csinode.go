...
// List lists all CSINodes in the indexer for a workspace.
func (s *cSINodeScopedLister) List(selector labels.Selector) (ret []*storagev1.CSINode, err error) {
...
// Get retrieves the CSINode from the indexer for a given workspace and name.
func (s *cSINodeScopedLister) Get(name string) (*storagev1.CSINode, error) {
	key := name
	obj, exists, err := s.indexer.GetByKey(key)
	if err != nil {
		return nil, err
