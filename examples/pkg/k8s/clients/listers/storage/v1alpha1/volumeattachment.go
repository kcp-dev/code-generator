...
// VolumeAttachmentLister can list all VolumeAttachments, or get one in particular.
// All objects returned here must be treated as read-only.
type VolumeAttachmentLister interface {
	// List lists all VolumeAttachments in the workspace.
	// Objects returned here must be treated as read-only.
	List(selector labels.Selector) (ret []*storagev1alpha1.VolumeAttachment, err error)
...
func NewVolumeAttachmentLister(indexer cache.Indexer) *volumeAttachmentScopedLister {
	return &volumeAttachmentScopedLister{indexer: indexer}
}
...
// List lists all VolumeAttachments in the indexer for a workspace.
func (s *volumeAttachmentScopedLister) List(selector labels.Selector) (ret []*storagev1alpha1.VolumeAttachment, err error) {
...
// Get retrieves the VolumeAttachment from the indexer for a given workspace and name.
func (s *volumeAttachmentScopedLister) Get(name string) (*storagev1alpha1.VolumeAttachment, error) {
	key := name
	obj, exists, err := s.indexer.GetByKey(key)
	if err != nil {
		return nil, err
