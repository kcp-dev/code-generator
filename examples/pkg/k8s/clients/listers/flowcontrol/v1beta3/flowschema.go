...
// FlowSchemaLister can list all FlowSchemas, or get one in particular.
// All objects returned here must be treated as read-only.
type FlowSchemaLister interface {
	// List lists all FlowSchemas in the workspace.
	// Objects returned here must be treated as read-only.
	List(selector labels.Selector) (ret []*flowcontrolv1beta3.FlowSchema, err error)
...
func NewFlowSchemaLister(indexer cache.Indexer) *flowSchemaScopedLister {
	return &flowSchemaScopedLister{indexer: indexer}
}
...
// List lists all FlowSchemas in the indexer for a workspace.
func (s *flowSchemaScopedLister) List(selector labels.Selector) (ret []*flowcontrolv1beta3.FlowSchema, err error) {
...
// Get retrieves the FlowSchema from the indexer for a given workspace and name.
func (s *flowSchemaScopedLister) Get(name string) (*flowcontrolv1beta3.FlowSchema, error) {
	key := name
	obj, exists, err := s.indexer.GetByKey(key)
	if err != nil {
		return nil, err
