...
// PriorityLevelConfigurationLister can list all PriorityLevelConfigurations, or get one in particular.
// All objects returned here must be treated as read-only.
type PriorityLevelConfigurationLister interface {
	// List lists all PriorityLevelConfigurations in the workspace.
	// Objects returned here must be treated as read-only.
	List(selector labels.Selector) (ret []*flowcontrolv1beta1.PriorityLevelConfiguration, err error)
...
func NewPriorityLevelConfigurationLister(indexer cache.Indexer) *priorityLevelConfigurationScopedLister {
	return &priorityLevelConfigurationScopedLister{indexer: indexer}
}
...
// List lists all PriorityLevelConfigurations in the indexer for a workspace.
func (s *priorityLevelConfigurationScopedLister) List(selector labels.Selector) (ret []*flowcontrolv1beta1.PriorityLevelConfiguration, err error) {
...
// Get retrieves the PriorityLevelConfiguration from the indexer for a given workspace and name.
func (s *priorityLevelConfigurationScopedLister) Get(name string) (*flowcontrolv1beta1.PriorityLevelConfiguration, error) {
	key := name
	obj, exists, err := s.indexer.GetByKey(key)
	if err != nil {
		return nil, err
