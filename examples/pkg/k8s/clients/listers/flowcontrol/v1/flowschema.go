...
// List lists all FlowSchemas in the indexer for a workspace.
func (s *flowSchemaScopedLister) List(selector labels.Selector) (ret []*flowcontrolv1.FlowSchema, err error) {
...
// Get retrieves the FlowSchema from the indexer for a given workspace and name.
func (s *flowSchemaScopedLister) Get(name string) (*flowcontrolv1.FlowSchema, error) {
	key := name
	obj, exists, err := s.indexer.GetByKey(key)
	if err != nil {
		return nil, err
