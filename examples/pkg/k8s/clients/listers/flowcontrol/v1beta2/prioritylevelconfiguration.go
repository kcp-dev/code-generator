...
// List lists all PriorityLevelConfigurations in the indexer for a workspace.
func (s *priorityLevelConfigurationScopedLister) List(selector labels.Selector) (ret []*flowcontrolv1beta2.PriorityLevelConfiguration, err error) {
...
// Get retrieves the PriorityLevelConfiguration from the indexer for a given workspace and name.
func (s *priorityLevelConfigurationScopedLister) Get(name string) (*flowcontrolv1beta2.PriorityLevelConfiguration, error) {
	key := name
	obj, exists, err := s.indexer.GetByKey(key)
	if err != nil {
		return nil, err
