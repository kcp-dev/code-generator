...
// List lists all RuntimeClasses in the indexer for a workspace.
func (s *runtimeClassScopedLister) List(selector labels.Selector) (ret []*nodev1.RuntimeClass, err error) {
...
// Get retrieves the RuntimeClass from the indexer for a given workspace and name.
func (s *runtimeClassScopedLister) Get(name string) (*nodev1.RuntimeClass, error) {
	key := name
	obj, exists, err := s.indexer.GetByKey(key)
	if err != nil {
		return nil, err
