...
// DeviceClassLister can list all DeviceClasses, or get one in particular.
// All objects returned here must be treated as read-only.
type DeviceClassLister interface {
	// List lists all DeviceClasses in the workspace.
	// Objects returned here must be treated as read-only.
	List(selector labels.Selector) (ret []*resourcev1alpha3.DeviceClass, err error)
...
func NewDeviceClassLister(indexer cache.Indexer) *deviceClassScopedLister {
	return &deviceClassScopedLister{indexer: indexer}
}
...
// List lists all DeviceClasses in the indexer for a workspace.
func (s *deviceClassScopedLister) List(selector labels.Selector) (ret []*resourcev1alpha3.DeviceClass, err error) {
...
// Get retrieves the DeviceClass from the indexer for a given workspace and name.
func (s *deviceClassScopedLister) Get(name string) (*resourcev1alpha3.DeviceClass, error) {
	key := name
	obj, exists, err := s.indexer.GetByKey(key)
	if err != nil {
		return nil, err
