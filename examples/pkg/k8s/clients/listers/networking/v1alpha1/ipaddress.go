...
// IPAddressLister can list all IPAddresses, or get one in particular.
// All objects returned here must be treated as read-only.
type IPAddressLister interface {
	// List lists all IPAddresses in the workspace.
	// Objects returned here must be treated as read-only.
	List(selector labels.Selector) (ret []*networkingv1alpha1.IPAddress, err error)
...
func NewIPAddressLister(indexer cache.Indexer) *iPAddressScopedLister {
	return &iPAddressScopedLister{indexer: indexer}
}
...
// List lists all IPAddresses in the indexer for a workspace.
func (s *iPAddressScopedLister) List(selector labels.Selector) (ret []*networkingv1alpha1.IPAddress, err error) {
...
// Get retrieves the IPAddress from the indexer for a given workspace and name.
func (s *iPAddressScopedLister) Get(name string) (*networkingv1alpha1.IPAddress, error) {
	key := name
	obj, exists, err := s.indexer.GetByKey(key)
	if err != nil {
		return nil, err
