...
// List lists all IngressClasses in the indexer for a workspace.
func (s *ingressClassScopedLister) List(selector labels.Selector) (ret []*networkingv1.IngressClass, err error) {
...
// Get retrieves the IngressClass from the indexer for a given workspace and name.
func (s *ingressClassScopedLister) Get(name string) (*networkingv1.IngressClass, error) {
	key := name
	obj, exists, err := s.indexer.GetByKey(key)
	if err != nil {
		return nil, err
