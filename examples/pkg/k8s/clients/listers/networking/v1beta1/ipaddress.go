...
// List lists all IPAddresses in the indexer for a workspace.
func (s *iPAddressScopedLister) List(selector labels.Selector) (ret []*networkingv1beta1.IPAddress, err error) {
...
// Get retrieves the IPAddress from the indexer for a given workspace and name.
func (s *iPAddressScopedLister) Get(name string) (*networkingv1beta1.IPAddress, error) {
	key := name
	obj, exists, err := s.indexer.GetByKey(key)
	if err != nil {
		return nil, err
