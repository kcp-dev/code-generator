...
// IngressClassLister can list all IngressClasses, or get one in particular.
// All objects returned here must be treated as read-only.
type IngressClassLister interface {
	// List lists all IngressClasses in the workspace.
	// Objects returned here must be treated as read-only.
	List(selector labels.Selector) (ret []*networkingv1beta1.IngressClass, err error)
...
func NewIngressClassLister(indexer cache.Indexer) *ingressClassScopedLister {
	return &ingressClassScopedLister{indexer: indexer}
}
...
// List lists all IngressClasses in the indexer for a workspace.
func (s *ingressClassScopedLister) List(selector labels.Selector) (ret []*networkingv1beta1.IngressClass, err error) {
...
// Get retrieves the IngressClass from the indexer for a given workspace and name.
func (s *ingressClassScopedLister) Get(name string) (*networkingv1beta1.IngressClass, error) {
	key := name
	obj, exists, err := s.indexer.GetByKey(key)
	if err != nil {
		return nil, err
