...
// List lists all PriorityClasses in the indexer for a workspace.
func (s *priorityClassScopedLister) List(selector labels.Selector) (ret []*schedulingv1beta1.PriorityClass, err error) {
...
// Get retrieves the PriorityClass from the indexer for a given workspace and name.
func (s *priorityClassScopedLister) Get(name string) (*schedulingv1beta1.PriorityClass, error) {
	key := name
	obj, exists, err := s.indexer.GetByKey(key)
	if err != nil {
		return nil, err
