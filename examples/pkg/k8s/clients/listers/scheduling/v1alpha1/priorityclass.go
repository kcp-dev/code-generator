...
type PriorityClassClusterLister interface {
	...
	Cluster(clusterName logicalcluster.Name) PriorityClassLister
...
// PriorityClassLister can list all PriorityClasses, or get one in particular.
// All objects returned here must be treated as read-only.
type PriorityClassLister interface {
	// List lists all PriorityClasses in the workspace.
	// Objects returned here must be treated as read-only.
	List(selector labels.Selector) (ret []*schedulingv1alpha1.PriorityClass, err error)
...
func NewPriorityClassLister(indexer cache.Indexer) *priorityClassScopedLister {
	return &priorityClassScopedLister{indexer: indexer}
}
...
// List lists all PriorityClasses in the indexer for a workspace.
func (s *priorityClassScopedLister) List(selector labels.Selector) (ret []*schedulingv1alpha1.PriorityClass, err error) {
...
// Get retrieves the PriorityClass from the indexer for a given workspace and name.
func (s *priorityClassScopedLister) Get(name string) (*schedulingv1alpha1.PriorityClass, error) {
	key := name
	obj, exists, err := s.indexer.GetByKey(key)
	if err != nil {
		return nil, err
