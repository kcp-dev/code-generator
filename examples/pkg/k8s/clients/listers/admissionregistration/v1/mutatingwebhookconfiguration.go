...
// List lists all MutatingWebhookConfigurations in the indexer for a workspace.
func (s *mutatingWebhookConfigurationScopedLister) List(selector labels.Selector) (ret []*admissionregistrationv1.MutatingWebhookConfiguration, err error) {
...
// Get retrieves the MutatingWebhookConfiguration from the indexer for a given workspace and name.
func (s *mutatingWebhookConfigurationScopedLister) Get(name string) (*admissionregistrationv1.MutatingWebhookConfiguration, error) {
	key := name
	obj, exists, err := s.indexer.GetByKey(key)
	if err != nil {
		return nil, err
