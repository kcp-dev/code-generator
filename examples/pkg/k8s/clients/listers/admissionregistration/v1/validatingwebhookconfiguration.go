...
// List lists all ValidatingWebhookConfigurations in the indexer for a workspace.
func (s *validatingWebhookConfigurationScopedLister) List(selector labels.Selector) (ret []*admissionregistrationv1.ValidatingWebhookConfiguration, err error) {
...
// Get retrieves the ValidatingWebhookConfiguration from the indexer for a given workspace and name.
func (s *validatingWebhookConfigurationScopedLister) Get(name string) (*admissionregistrationv1.ValidatingWebhookConfiguration, error) {
	key := name
	obj, exists, err := s.indexer.GetByKey(key)
	if err != nil {
		return nil, err
