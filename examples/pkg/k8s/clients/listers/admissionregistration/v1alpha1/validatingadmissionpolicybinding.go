...
// List lists all ValidatingAdmissionPolicyBindings in the indexer for a workspace.
func (s *validatingAdmissionPolicyBindingScopedLister) List(selector labels.Selector) (ret []*admissionregistrationv1alpha1.ValidatingAdmissionPolicyBinding, err error) {
...
// Get retrieves the ValidatingAdmissionPolicyBinding from the indexer for a given workspace and name.
func (s *validatingAdmissionPolicyBindingScopedLister) Get(name string) (*admissionregistrationv1alpha1.ValidatingAdmissionPolicyBinding, error) {
	key := name
	obj, exists, err := s.indexer.GetByKey(key)
	if err != nil {
		return nil, err
