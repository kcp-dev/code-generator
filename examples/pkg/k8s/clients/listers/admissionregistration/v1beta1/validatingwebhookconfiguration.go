...
// ValidatingWebhookConfigurationLister can list all ValidatingWebhookConfigurations, or get one in particular.
// All objects returned here must be treated as read-only.
type ValidatingWebhookConfigurationLister interface {
	// List lists all ValidatingWebhookConfigurations in the workspace.
	// Objects returned here must be treated as read-only.
	List(selector labels.Selector) (ret []*admissionregistrationv1beta1.ValidatingWebhookConfiguration, err error)
...
func NewValidatingWebhookConfigurationLister(indexer cache.Indexer) *validatingWebhookConfigurationScopedLister {
	return &validatingWebhookConfigurationScopedLister{indexer: indexer}
}
...
// List lists all ValidatingWebhookConfigurations in the indexer for a workspace.
func (s *validatingWebhookConfigurationScopedLister) List(selector labels.Selector) (ret []*admissionregistrationv1beta1.ValidatingWebhookConfiguration, err error) {
...
// Get retrieves the ValidatingWebhookConfiguration from the indexer for a given workspace and name.
func (s *validatingWebhookConfigurationScopedLister) Get(name string) (*admissionregistrationv1beta1.ValidatingWebhookConfiguration, error) {
	key := name
	obj, exists, err := s.indexer.GetByKey(key)
	if err != nil {
		return nil, err
