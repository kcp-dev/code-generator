...
// List lists all ValidatingAdmissionPolicies in the indexer for a workspace.
func (s *validatingAdmissionPolicyScopedLister) List(selector labels.Selector) (ret []*admissionregistrationv1beta1.ValidatingAdmissionPolicy, err error) {
...
// Get retrieves the ValidatingAdmissionPolicy from the indexer for a given workspace and name.
func (s *validatingAdmissionPolicyScopedLister) Get(name string) (*admissionregistrationv1beta1.ValidatingAdmissionPolicy, error) {
	key := name
	obj, exists, err := s.indexer.GetByKey(key)
	if err != nil {
		return nil, err
