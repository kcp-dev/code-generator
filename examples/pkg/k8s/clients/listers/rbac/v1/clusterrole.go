...
// List lists all ClusterRoles in the indexer for a workspace.
func (s *clusterRoleScopedLister) List(selector labels.Selector) (ret []*rbacv1.ClusterRole, err error) {
...
// Get retrieves the ClusterRole from the indexer for a given workspace and name.
func (s *clusterRoleScopedLister) Get(name string) (*rbacv1.ClusterRole, error) {
	key := name
	obj, exists, err := s.indexer.GetByKey(key)
	if err != nil {
		return nil, err
