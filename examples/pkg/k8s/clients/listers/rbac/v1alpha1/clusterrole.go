...
// ClusterRoleLister can list all ClusterRoles, or get one in particular.
// All objects returned here must be treated as read-only.
type ClusterRoleLister interface {
	// List lists all ClusterRoles in the workspace.
	// Objects returned here must be treated as read-only.
	List(selector labels.Selector) (ret []*rbacv1alpha1.ClusterRole, err error)
...
func NewClusterRoleLister(indexer cache.Indexer) *clusterRoleScopedLister {
	return &clusterRoleScopedLister{indexer: indexer}
}
...
// List lists all ClusterRoles in the indexer for a workspace.
func (s *clusterRoleScopedLister) List(selector labels.Selector) (ret []*rbacv1alpha1.ClusterRole, err error) {
...
// Get retrieves the ClusterRole from the indexer for a given workspace and name.
func (s *clusterRoleScopedLister) Get(name string) (*rbacv1alpha1.ClusterRole, error) {
	key := name
	obj, exists, err := s.indexer.GetByKey(key)
	if err != nil {
		return nil, err
