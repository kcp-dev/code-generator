...
// ClusterRoleBindingLister can list all ClusterRoleBindings, or get one in particular.
// All objects returned here must be treated as read-only.
type ClusterRoleBindingLister interface {
	// List lists all ClusterRoleBindings in the workspace.
	// Objects returned here must be treated as read-only.
	List(selector labels.Selector) (ret []*rbacv1alpha1.ClusterRoleBinding, err error)
...
func NewClusterRoleBindingLister(indexer cache.Indexer) *clusterRoleBindingScopedLister {
	return &clusterRoleBindingScopedLister{indexer: indexer}
}
...
// List lists all ClusterRoleBindings in the indexer for a workspace.
func (s *clusterRoleBindingScopedLister) List(selector labels.Selector) (ret []*rbacv1alpha1.ClusterRoleBinding, err error) {
...
// Get retrieves the ClusterRoleBinding from the indexer for a given workspace and name.
func (s *clusterRoleBindingScopedLister) Get(name string) (*rbacv1alpha1.ClusterRoleBinding, error) {
	key := name
	obj, exists, err := s.indexer.GetByKey(key)
	if err != nil {
		return nil, err
