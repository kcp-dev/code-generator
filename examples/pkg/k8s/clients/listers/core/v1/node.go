...
// NodeLister can list all Nodes, or get one in particular.
// All objects returned here must be treated as read-only.
type NodeLister interface {
	// List lists all Nodes in the workspace.
	// Objects returned here must be treated as read-only.
	List(selector labels.Selector) (ret []*corev1.Node, err error)
...
func NewNodeLister(indexer cache.Indexer) *nodeScopedLister {
	return &nodeScopedLister{indexer: indexer}
}
...
// List lists all Nodes in the indexer for a workspace.
func (s *nodeScopedLister) List(selector labels.Selector) (ret []*corev1.Node, err error) {
...
// Get retrieves the Node from the indexer for a given workspace and name.
func (s *nodeScopedLister) Get(name string) (*corev1.Node, error) {
	key := name
	obj, exists, err := s.indexer.GetByKey(key)
	if err != nil {
		return nil, err
