...
// ComponentStatusLister can list all ComponentStatuses, or get one in particular.
// All objects returned here must be treated as read-only.
type ComponentStatusLister interface {
	// List lists all ComponentStatuses in the workspace.
	// Objects returned here must be treated as read-only.
	List(selector labels.Selector) (ret []*corev1.ComponentStatus, err error)
...
func NewComponentStatusLister(indexer cache.Indexer) *componentStatusScopedLister {
	return &componentStatusScopedLister{indexer: indexer}
}
...
// List lists all ComponentStatuses in the indexer for a workspace.
func (s *componentStatusScopedLister) List(selector labels.Selector) (ret []*corev1.ComponentStatus, err error) {
...
// Get retrieves the ComponentStatus from the indexer for a given workspace and name.
func (s *componentStatusScopedLister) Get(name string) (*corev1.ComponentStatus, error) {
	key := name
	obj, exists, err := s.indexer.GetByKey(key)
	if err != nil {
		return nil, err
