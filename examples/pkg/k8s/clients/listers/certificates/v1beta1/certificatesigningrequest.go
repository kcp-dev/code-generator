...
type CertificateSigningRequestClusterLister interface {
	...
	Cluster(clusterName logicalcluster.Name) CertificateSigningRequestLister
...
// CertificateSigningRequestLister can list all CertificateSigningRequests, or get one in particular.
// All objects returned here must be treated as read-only.
type CertificateSigningRequestLister interface {
	// List lists all CertificateSigningRequests in the workspace.
	// Objects returned here must be treated as read-only.
	List(selector labels.Selector) (ret []*certificatesv1beta1.CertificateSigningRequest, err error)
...
func NewCertificateSigningRequestLister(indexer cache.Indexer) *certificateSigningRequestScopedLister {
	return &certificateSigningRequestScopedLister{indexer: indexer}
}
...
// List lists all CertificateSigningRequests in the indexer for a workspace.
func (s *certificateSigningRequestScopedLister) List(selector labels.Selector) (ret []*certificatesv1beta1.CertificateSigningRequest, err error) {
...
// Get retrieves the CertificateSigningRequest from the indexer for a given workspace and name.
func (s *certificateSigningRequestScopedLister) Get(name string) (*certificatesv1beta1.CertificateSigningRequest, error) {
	key := name
	obj, exists, err := s.indexer.GetByKey(key)
	if err != nil {
		return nil, err
