...
type FlowSchemaInformer interface {
	...
	Lister() flowcontrolv1beta3listers.FlowSchemaLister
}

func (f *flowSchemaClusterInformer) Cluster(clusterName logicalcluster.Name) FlowSchemaInformer {
...
func (f *flowSchemaInformer) Lister() flowcontrolv1beta3listers.FlowSchemaLister {
	return f.lister
}

type flowSchemaScopedInformer struct {
	factory          internalinterfaces.SharedScopedInformerFactory
	tweakListOptions internalinterfaces.TweakListOptionsFunc
}

func (f *flowSchemaScopedInformer) Informer() cache.SharedIndexInformer {
	return f.factory.InformerFor(&flowcontrolv1beta3.FlowSchema{}, f.defaultInformer)
}

func (f *flowSchemaScopedInformer) Lister() flowcontrolv1beta3listers.FlowSchemaLister {
	return flowcontrolv1beta3listers.NewFlowSchemaLister(f.Informer().GetIndexer())
}

// NewFlowSchemaInformer constructs a new informer for FlowSchema type.
// Always prefer using an informer factory to get a shared informer instead of getting an independent
// one. This reduces memory footprint and number of connections to the server.
func NewFlowSchemaInformer(client scopedclientset.Interface, resyncPeriod time.Duration, indexers cache.Indexers) cache.SharedIndexInformer {
...
// Always prefer using an informer factory to get a shared informer instead of getting an independent
// one. This reduces memory footprint and number of connections to the server.
func NewFilteredFlowSchemaInformer(client scopedclientset.Interface, resyncPeriod time.Duration, indexers cache.Indexers, tweakListOptions internalinterfaces.TweakListOptionsFunc) cache.SharedIndexInformer {
...
func (f *flowSchemaScopedInformer) defaultInformer(client scopedclientset.Interface, resyncPeriod time.Duration) cache.SharedIndexInformer {
	return NewFilteredFlowSchemaInformer(client, resyncPeriod, cache.Indexers{}, f.tweakListOptions)
}
