...
import (
	...
	certificatesv1beta1listers "acme.corp/pkg/kcp/clients/listers/certificates/v1beta1"
)
...
type CertificateSigningRequestInformer interface {
	...
	Lister() certificatesv1beta1listers.CertificateSigningRequestLister
}

func (f *certificateSigningRequestClusterInformer) Cluster(clusterName logicalcluster.Name) CertificateSigningRequestInformer {
...
type certificateSigningRequestScopedInformer struct {
	factory          internalinterfaces.SharedScopedInformerFactory
	tweakListOptions internalinterfaces.TweakListOptionsFunc
}

func (f *certificateSigningRequestScopedInformer) Informer() cache.SharedIndexInformer {
	return f.factory.InformerFor(&certificatesv1beta1.CertificateSigningRequest{}, f.defaultInformer)
}

func (f *certificateSigningRequestScopedInformer) Lister() certificatesv1beta1listers.CertificateSigningRequestLister {
	return certificatesv1beta1listers.NewCertificateSigningRequestLister(f.Informer().GetIndexer())
}

// NewCertificateSigningRequestInformer constructs a new informer for CertificateSigningRequest type.
// Always prefer using an informer factory to get a shared informer instead of getting an independent
// one. This reduces memory footprint and number of connections to the server.
func NewCertificateSigningRequestInformer(client scopedclientset.Interface, resyncPeriod time.Duration, indexers cache.Indexers) cache.SharedIndexInformer {
...
// Always prefer using an informer factory to get a shared informer instead of getting an independent
// one. This reduces memory footprint and number of connections to the server.
func NewFilteredCertificateSigningRequestInformer(client scopedclientset.Interface, resyncPeriod time.Duration, indexers cache.Indexers, tweakListOptions internalinterfaces.TweakListOptionsFunc) cache.SharedIndexInformer {
...
func (f *certificateSigningRequestScopedInformer) defaultInformer(client scopedclientset.Interface, resyncPeriod time.Duration) cache.SharedIndexInformer {
	return NewFilteredCertificateSigningRequestInformer(client, resyncPeriod, cache.Indexers{}, f.tweakListOptions)
}
