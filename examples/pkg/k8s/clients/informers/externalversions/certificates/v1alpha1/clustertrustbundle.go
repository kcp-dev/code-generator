...
import (
	...
	certificatesv1alpha1listers "acme.corp/pkg/kcp/clients/listers/certificates/v1alpha1"
)
...
type ClusterTrustBundleInformer interface {
	...
	Lister() certificatesv1alpha1listers.ClusterTrustBundleLister
}

func (f *clusterTrustBundleClusterInformer) Cluster(clusterName logicalcluster.Name) ClusterTrustBundleInformer {
...
func (f *clusterTrustBundleInformer) Lister() certificatesv1alpha1listers.ClusterTrustBundleLister {
	return f.lister
}

type clusterTrustBundleScopedInformer struct {
	factory          internalinterfaces.SharedScopedInformerFactory
	tweakListOptions internalinterfaces.TweakListOptionsFunc
}

func (f *clusterTrustBundleScopedInformer) Informer() cache.SharedIndexInformer {
	return f.factory.InformerFor(&certificatesv1alpha1.ClusterTrustBundle{}, f.defaultInformer)
}

func (f *clusterTrustBundleScopedInformer) Lister() certificatesv1alpha1listers.ClusterTrustBundleLister {
	return certificatesv1alpha1listers.NewClusterTrustBundleLister(f.Informer().GetIndexer())
}

// NewClusterTrustBundleInformer constructs a new informer for ClusterTrustBundle type.
// Always prefer using an informer factory to get a shared informer instead of getting an independent
// one. This reduces memory footprint and number of connections to the server.
func NewClusterTrustBundleInformer(client scopedclientset.Interface, resyncPeriod time.Duration, indexers cache.Indexers) cache.SharedIndexInformer {
...
// Always prefer using an informer factory to get a shared informer instead of getting an independent
// one. This reduces memory footprint and number of connections to the server.
func NewFilteredClusterTrustBundleInformer(client scopedclientset.Interface, resyncPeriod time.Duration, indexers cache.Indexers, tweakListOptions internalinterfaces.TweakListOptionsFunc) cache.SharedIndexInformer {
...
func (f *clusterTrustBundleScopedInformer) defaultInformer(client scopedclientset.Interface, resyncPeriod time.Duration) cache.SharedIndexInformer {
	return NewFilteredClusterTrustBundleInformer(client, resyncPeriod, cache.Indexers{}, f.tweakListOptions)
}
