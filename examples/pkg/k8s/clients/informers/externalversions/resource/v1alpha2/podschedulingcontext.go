...
type PodSchedulingContextInformer interface {
	...
	Lister() resourcev1alpha2listers.PodSchedulingContextLister
}

func (f *podSchedulingContextClusterInformer) Cluster(clusterName logicalcluster.Name) PodSchedulingContextInformer {
...
func (f *podSchedulingContextInformer) Lister() resourcev1alpha2listers.PodSchedulingContextLister {
	return f.lister
}

type podSchedulingContextScopedInformer struct {
	factory          internalinterfaces.SharedScopedInformerFactory
	tweakListOptions internalinterfaces.TweakListOptionsFunc
	namespace        string
}

func (f *podSchedulingContextScopedInformer) Informer() cache.SharedIndexInformer {
	return f.factory.InformerFor(&resourcev1alpha2.PodSchedulingContext{}, f.defaultInformer)
}

func (f *podSchedulingContextScopedInformer) Lister() resourcev1alpha2listers.PodSchedulingContextLister {
	return resourcev1alpha2listers.NewPodSchedulingContextLister(f.Informer().GetIndexer())
}

// NewPodSchedulingContextInformer constructs a new informer for PodSchedulingContext type.
// Always prefer using an informer factory to get a shared informer instead of getting an independent
// one. This reduces memory footprint and number of connections to the server.
func NewPodSchedulingContextInformer(client scopedclientset.Interface, resyncPeriod time.Duration, namespace string, indexers cache.Indexers) cache.SharedIndexInformer {
	return NewFilteredPodSchedulingContextInformer(client, resyncPeriod, namespace, indexers, nil)
}

// NewFilteredPodSchedulingContextInformer constructs a new informer for PodSchedulingContext type.
// Always prefer using an informer factory to get a shared informer instead of getting an independent
// one. This reduces memory footprint and number of connections to the server.
func NewFilteredPodSchedulingContextInformer(client scopedclientset.Interface, resyncPeriod time.Duration, namespace string, indexers cache.Indexers, tweakListOptions internalinterfaces.TweakListOptionsFunc) cache.SharedIndexInformer {
...
func (f *podSchedulingContextScopedInformer) defaultInformer(client scopedclientset.Interface, resyncPeriod time.Duration) cache.SharedIndexInformer {
	return NewFilteredPodSchedulingContextInformer(client, resyncPeriod, f.namespace, cache.Indexers{
