...
type DeviceClassInformer interface {
	...
	Lister() resourcev1alpha3listers.DeviceClassLister
}

func (f *deviceClassClusterInformer) Cluster(clusterName logicalcluster.Name) DeviceClassInformer {
...
func (f *deviceClassInformer) Lister() resourcev1alpha3listers.DeviceClassLister {
	return f.lister
}

type deviceClassScopedInformer struct {
	factory          internalinterfaces.SharedScopedInformerFactory
	tweakListOptions internalinterfaces.TweakListOptionsFunc
}

func (f *deviceClassScopedInformer) Informer() cache.SharedIndexInformer {
	return f.factory.InformerFor(&resourcev1alpha3.DeviceClass{}, f.defaultInformer)
}

func (f *deviceClassScopedInformer) Lister() resourcev1alpha3listers.DeviceClassLister {
	return resourcev1alpha3listers.NewDeviceClassLister(f.Informer().GetIndexer())
}

// NewDeviceClassInformer constructs a new informer for DeviceClass type.
// Always prefer using an informer factory to get a shared informer instead of getting an independent
// one. This reduces memory footprint and number of connections to the server.
func NewDeviceClassInformer(client scopedclientset.Interface, resyncPeriod time.Duration, indexers cache.Indexers) cache.SharedIndexInformer {
...
// Always prefer using an informer factory to get a shared informer instead of getting an independent
// one. This reduces memory footprint and number of connections to the server.
func NewFilteredDeviceClassInformer(client scopedclientset.Interface, resyncPeriod time.Duration, indexers cache.Indexers, tweakListOptions internalinterfaces.TweakListOptionsFunc) cache.SharedIndexInformer {
...
func (f *deviceClassScopedInformer) defaultInformer(client scopedclientset.Interface, resyncPeriod time.Duration) cache.SharedIndexInformer {
	return NewFilteredDeviceClassInformer(client, resyncPeriod, cache.Indexers{}, f.tweakListOptions)
}
