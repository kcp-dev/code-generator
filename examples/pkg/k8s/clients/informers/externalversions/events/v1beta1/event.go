...
import (
	...
	eventsv1beta1listers "acme.corp/pkg/kcp/clients/listers/events/v1beta1"
)
...
type EventInformer interface {
	...
	Lister() eventsv1beta1listers.EventLister
}

func (f *eventClusterInformer) Cluster(clusterName logicalcluster.Name) EventInformer {
...
type eventScopedInformer struct {
	factory          internalinterfaces.SharedScopedInformerFactory
	tweakListOptions internalinterfaces.TweakListOptionsFunc
	namespace        string
}

func (f *eventScopedInformer) Informer() cache.SharedIndexInformer {
	return f.factory.InformerFor(&eventsv1beta1.Event{}, f.defaultInformer)
}

func (f *eventScopedInformer) Lister() eventsv1beta1listers.EventLister {
	return eventsv1beta1listers.NewEventLister(f.Informer().GetIndexer())
}

// NewEventInformer constructs a new informer for Event type.
// Always prefer using an informer factory to get a shared informer instead of getting an independent
// one. This reduces memory footprint and number of connections to the server.
func NewEventInformer(client scopedclientset.Interface, resyncPeriod time.Duration, namespace string, indexers cache.Indexers) cache.SharedIndexInformer {
	return NewFilteredEventInformer(client, resyncPeriod, namespace, indexers, nil)
}

// NewFilteredEventInformer constructs a new informer for Event type.
// Always prefer using an informer factory to get a shared informer instead of getting an independent
// one. This reduces memory footprint and number of connections to the server.
func NewFilteredEventInformer(client scopedclientset.Interface, resyncPeriod time.Duration, namespace string, indexers cache.Indexers, tweakListOptions internalinterfaces.TweakListOptionsFunc) cache.SharedIndexInformer {
...
func (f *eventScopedInformer) defaultInformer(client scopedclientset.Interface, resyncPeriod time.Duration) cache.SharedIndexInformer {
	return NewFilteredEventInformer(client, resyncPeriod, f.namespace, cache.Indexers{
