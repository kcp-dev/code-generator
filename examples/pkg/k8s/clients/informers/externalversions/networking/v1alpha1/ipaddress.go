...
type IPAddressInformer interface {
	...
	Lister() networkingv1alpha1listers.IPAddressLister
}

func (f *iPAddressClusterInformer) Cluster(clusterName logicalcluster.Name) IPAddressInformer {
...
func (f *iPAddressInformer) Lister() networkingv1alpha1listers.IPAddressLister {
	return f.lister
}

type iPAddressScopedInformer struct {
	factory          internalinterfaces.SharedScopedInformerFactory
	tweakListOptions internalinterfaces.TweakListOptionsFunc
}

func (f *iPAddressScopedInformer) Informer() cache.SharedIndexInformer {
	return f.factory.InformerFor(&networkingv1alpha1.IPAddress{}, f.defaultInformer)
}

func (f *iPAddressScopedInformer) Lister() networkingv1alpha1listers.IPAddressLister {
	return networkingv1alpha1listers.NewIPAddressLister(f.Informer().GetIndexer())
}

// NewIPAddressInformer constructs a new informer for IPAddress type.
// Always prefer using an informer factory to get a shared informer instead of getting an independent
// one. This reduces memory footprint and number of connections to the server.
func NewIPAddressInformer(client scopedclientset.Interface, resyncPeriod time.Duration, indexers cache.Indexers) cache.SharedIndexInformer {
...
// Always prefer using an informer factory to get a shared informer instead of getting an independent
// one. This reduces memory footprint and number of connections to the server.
func NewFilteredIPAddressInformer(client scopedclientset.Interface, resyncPeriod time.Duration, indexers cache.Indexers, tweakListOptions internalinterfaces.TweakListOptionsFunc) cache.SharedIndexInformer {
...
func (f *iPAddressScopedInformer) defaultInformer(client scopedclientset.Interface, resyncPeriod time.Duration) cache.SharedIndexInformer {
	return NewFilteredIPAddressInformer(client, resyncPeriod, cache.Indexers{}, f.tweakListOptions)
}
