...
import (
	...
	storagev1listers "acme.corp/pkg/kcp/clients/listers/storage/v1"
)
...
type CSIStorageCapacityInformer interface {
	...
	Lister() storagev1listers.CSIStorageCapacityLister
}

func (f *cSIStorageCapacityClusterInformer) Cluster(clusterName logicalcluster.Name) CSIStorageCapacityInformer {
...
type cSIStorageCapacityScopedInformer struct {
	factory          internalinterfaces.SharedScopedInformerFactory
	tweakListOptions internalinterfaces.TweakListOptionsFunc
	namespace        string
}

func (f *cSIStorageCapacityScopedInformer) Informer() cache.SharedIndexInformer {
	return f.factory.InformerFor(&storagev1.CSIStorageCapacity{}, f.defaultInformer)
}

func (f *cSIStorageCapacityScopedInformer) Lister() storagev1listers.CSIStorageCapacityLister {
	return storagev1listers.NewCSIStorageCapacityLister(f.Informer().GetIndexer())
}

// NewCSIStorageCapacityInformer constructs a new informer for CSIStorageCapacity type.
// Always prefer using an informer factory to get a shared informer instead of getting an independent
// one. This reduces memory footprint and number of connections to the server.
func NewCSIStorageCapacityInformer(client scopedclientset.Interface, resyncPeriod time.Duration, namespace string, indexers cache.Indexers) cache.SharedIndexInformer {
	return NewFilteredCSIStorageCapacityInformer(client, resyncPeriod, namespace, indexers, nil)
}

// NewFilteredCSIStorageCapacityInformer constructs a new informer for CSIStorageCapacity type.
// Always prefer using an informer factory to get a shared informer instead of getting an independent
// one. This reduces memory footprint and number of connections to the server.
func NewFilteredCSIStorageCapacityInformer(client scopedclientset.Interface, resyncPeriod time.Duration, namespace string, indexers cache.Indexers, tweakListOptions internalinterfaces.TweakListOptionsFunc) cache.SharedIndexInformer {
...
func (f *cSIStorageCapacityScopedInformer) defaultInformer(client scopedclientset.Interface, resyncPeriod time.Duration) cache.SharedIndexInformer {
	return NewFilteredCSIStorageCapacityInformer(client, resyncPeriod, f.namespace, cache.Indexers{
