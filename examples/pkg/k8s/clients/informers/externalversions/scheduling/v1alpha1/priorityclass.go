...
import (
	...
	schedulingv1alpha1listers "acme.corp/pkg/kcp/clients/listers/scheduling/v1alpha1"
)
...
type PriorityClassInformer interface {
	...
	Lister() schedulingv1alpha1listers.PriorityClassLister
}

func (f *priorityClassClusterInformer) Cluster(clusterName logicalcluster.Name) PriorityClassInformer {
...
type priorityClassScopedInformer struct {
	factory          internalinterfaces.SharedScopedInformerFactory
	tweakListOptions internalinterfaces.TweakListOptionsFunc
}

func (f *priorityClassScopedInformer) Informer() cache.SharedIndexInformer {
	return f.factory.InformerFor(&schedulingv1alpha1.PriorityClass{}, f.defaultInformer)
}

func (f *priorityClassScopedInformer) Lister() schedulingv1alpha1listers.PriorityClassLister {
	return schedulingv1alpha1listers.NewPriorityClassLister(f.Informer().GetIndexer())
}

// NewPriorityClassInformer constructs a new informer for PriorityClass type.
// Always prefer using an informer factory to get a shared informer instead of getting an independent
// one. This reduces memory footprint and number of connections to the server.
func NewPriorityClassInformer(client scopedclientset.Interface, resyncPeriod time.Duration, indexers cache.Indexers) cache.SharedIndexInformer {
...
// Always prefer using an informer factory to get a shared informer instead of getting an independent
// one. This reduces memory footprint and number of connections to the server.
func NewFilteredPriorityClassInformer(client scopedclientset.Interface, resyncPeriod time.Duration, indexers cache.Indexers, tweakListOptions internalinterfaces.TweakListOptionsFunc) cache.SharedIndexInformer {
...
func (f *priorityClassScopedInformer) defaultInformer(client scopedclientset.Interface, resyncPeriod time.Duration) cache.SharedIndexInformer {
