...
import (
	...
	appsv1beta2listers "acme.corp/pkg/kcp/clients/listers/apps/v1beta2"
)
...
type ControllerRevisionInformer interface {
	...
	Lister() appsv1beta2listers.ControllerRevisionLister
}

func (f *controllerRevisionClusterInformer) Cluster(clusterName logicalcluster.Name) ControllerRevisionInformer {
...
type controllerRevisionScopedInformer struct {
	factory          internalinterfaces.SharedScopedInformerFactory
	tweakListOptions internalinterfaces.TweakListOptionsFunc
	namespace        string
}

func (f *controllerRevisionScopedInformer) Informer() cache.SharedIndexInformer {
	return f.factory.InformerFor(&appsv1beta2.ControllerRevision{}, f.defaultInformer)
}

func (f *controllerRevisionScopedInformer) Lister() appsv1beta2listers.ControllerRevisionLister {
	return appsv1beta2listers.NewControllerRevisionLister(f.Informer().GetIndexer())
}

// NewControllerRevisionInformer constructs a new informer for ControllerRevision type.
// Always prefer using an informer factory to get a shared informer instead of getting an independent
// one. This reduces memory footprint and number of connections to the server.
func NewControllerRevisionInformer(client scopedclientset.Interface, resyncPeriod time.Duration, namespace string, indexers cache.Indexers) cache.SharedIndexInformer {
	return NewFilteredControllerRevisionInformer(client, resyncPeriod, namespace, indexers, nil)
}

// NewFilteredControllerRevisionInformer constructs a new informer for ControllerRevision type.
// Always prefer using an informer factory to get a shared informer instead of getting an independent
// one. This reduces memory footprint and number of connections to the server.
func NewFilteredControllerRevisionInformer(client scopedclientset.Interface, resyncPeriod time.Duration, namespace string, indexers cache.Indexers, tweakListOptions internalinterfaces.TweakListOptionsFunc) cache.SharedIndexInformer {
...
func (f *controllerRevisionScopedInformer) defaultInformer(client scopedclientset.Interface, resyncPeriod time.Duration) cache.SharedIndexInformer {
	return NewFilteredControllerRevisionInformer(client, resyncPeriod, f.namespace, cache.Indexers{
