...
import (
	...
	batchv1beta1listers "acme.corp/pkg/kcp/clients/listers/batch/v1beta1"
)
...
type CronJobInformer interface {
	...
	Lister() batchv1beta1listers.CronJobLister
}

func (f *cronJobClusterInformer) Cluster(clusterName logicalcluster.Name) CronJobInformer {
...
type cronJobScopedInformer struct {
	factory          internalinterfaces.SharedScopedInformerFactory
	tweakListOptions internalinterfaces.TweakListOptionsFunc
	namespace        string
}

func (f *cronJobScopedInformer) Informer() cache.SharedIndexInformer {
	return f.factory.InformerFor(&batchv1beta1.CronJob{}, f.defaultInformer)
}

func (f *cronJobScopedInformer) Lister() batchv1beta1listers.CronJobLister {
	return batchv1beta1listers.NewCronJobLister(f.Informer().GetIndexer())
}

// NewCronJobInformer constructs a new informer for CronJob type.
// Always prefer using an informer factory to get a shared informer instead of getting an independent
// one. This reduces memory footprint and number of connections to the server.
func NewCronJobInformer(client scopedclientset.Interface, resyncPeriod time.Duration, namespace string, indexers cache.Indexers) cache.SharedIndexInformer {
	return NewFilteredCronJobInformer(client, resyncPeriod, namespace, indexers, nil)
}

// NewFilteredCronJobInformer constructs a new informer for CronJob type.
// Always prefer using an informer factory to get a shared informer instead of getting an independent
// one. This reduces memory footprint and number of connections to the server.
func NewFilteredCronJobInformer(client scopedclientset.Interface, resyncPeriod time.Duration, namespace string, indexers cache.Indexers, tweakListOptions internalinterfaces.TweakListOptionsFunc) cache.SharedIndexInformer {
...
func (f *cronJobScopedInformer) defaultInformer(client scopedclientset.Interface, resyncPeriod time.Duration) cache.SharedIndexInformer {
	return NewFilteredCronJobInformer(client, resyncPeriod, f.namespace, cache.Indexers{
