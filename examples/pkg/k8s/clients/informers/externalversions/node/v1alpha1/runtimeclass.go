...
import (
	...
	nodev1alpha1listers "acme.corp/pkg/kcp/clients/listers/node/v1alpha1"
)
...
type RuntimeClassInformer interface {
	...
	Lister() nodev1alpha1listers.RuntimeClassLister
}

func (f *runtimeClassClusterInformer) Cluster(clusterName logicalcluster.Name) RuntimeClassInformer {
...
type runtimeClassScopedInformer struct {
	factory          internalinterfaces.SharedScopedInformerFactory
	tweakListOptions internalinterfaces.TweakListOptionsFunc
}

func (f *runtimeClassScopedInformer) Informer() cache.SharedIndexInformer {
	return f.factory.InformerFor(&nodev1alpha1.RuntimeClass{}, f.defaultInformer)
}

func (f *runtimeClassScopedInformer) Lister() nodev1alpha1listers.RuntimeClassLister {
	return nodev1alpha1listers.NewRuntimeClassLister(f.Informer().GetIndexer())
}

// NewRuntimeClassInformer constructs a new informer for RuntimeClass type.
// Always prefer using an informer factory to get a shared informer instead of getting an independent
// one. This reduces memory footprint and number of connections to the server.
func NewRuntimeClassInformer(client scopedclientset.Interface, resyncPeriod time.Duration, indexers cache.Indexers) cache.SharedIndexInformer {
...
// Always prefer using an informer factory to get a shared informer instead of getting an independent
// one. This reduces memory footprint and number of connections to the server.
func NewFilteredRuntimeClassInformer(client scopedclientset.Interface, resyncPeriod time.Duration, indexers cache.Indexers, tweakListOptions internalinterfaces.TweakListOptionsFunc) cache.SharedIndexInformer {
...
func (f *runtimeClassScopedInformer) defaultInformer(client scopedclientset.Interface, resyncPeriod time.Duration) cache.SharedIndexInformer {
