...
func (c *leasesClient) ApplyStatus(ctx context.Context, applyConfiguration *coordinationv1beta1.LeaseApplyConfiguration, opts metav1.ApplyOptions) (*v1beta1.Lease, error) {
	if applyConfiguration == nil {
		return nil, fmt.Errorf("applyConfiguration provided to Apply must not be nil")
	}
	data, err := json.Marshal(applyConfiguration)
	...
	obj, err := c.Fake.Invokes(kcptesting.NewPatchSubresourceAction(leasesResource, c.ClusterPath, c.Namespace, *name, types.ApplyPatchType, data, "status"), &v1beta1.Lease{})
