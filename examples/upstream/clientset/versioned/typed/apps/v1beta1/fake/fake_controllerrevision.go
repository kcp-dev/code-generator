...
func (c *controllerRevisionsClient) ApplyStatus(ctx context.Context, applyConfiguration *appsv1beta1.ControllerRevisionApplyConfiguration, opts metav1.ApplyOptions) (*v1beta1.ControllerRevision, error) {
	if applyConfiguration == nil {
		return nil, fmt.Errorf("applyConfiguration provided to Apply must not be nil")
	}
	data, err := json.Marshal(applyConfiguration)
	...
	obj, err := c.Fake.Invokes(kcptesting.NewPatchSubresourceAction(controllerrevisionsResource, c.ClusterPath, c.Namespace, *name, types.ApplyPatchType, data, "status"), &v1beta1.ControllerRevision{})
