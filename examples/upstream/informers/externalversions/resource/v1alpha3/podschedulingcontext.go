...
type PodSchedulingContextClusterInformer interface {
	...
	Cluster(logicalcluster.Name) upstreamresource.k8s.iov1alpha3informers.PodSchedulingContextInformer
}
...
// Always prefer using an informer factory to get a shared informer instead of getting an independent
// one. This reduces memory footprint and number of connections to the server.
func NewFilteredPodSchedulingContextClusterInformer(client versioned.ClusterInterface, resyncPeriod time.Duration, indexers cache.Indexers, tweakListOptions internalinterfaces.TweakListOptionsFunc) cache.SharedIndexInformer {
...
func (f *podSchedulingContextClusterInformer) defaultInformer(client versioned.Interface, resyncPeriod time.Duration) cache.SharedIndexInformer {
	return NewFilteredPodSchedulingContextClusterInformer(client, f.namespace, resyncPeriod, cache.Indexers{
