...
type CSIStorageCapacityClusterInformer interface {
	...
	Cluster(logicalcluster.Name) upstreamstorage.k8s.iov1alpha1informers.CSIStorageCapacityInformer
}
...
// Always prefer using an informer factory to get a shared informer instead of getting an independent
// one. This reduces memory footprint and number of connections to the server.
func NewFilteredCSIStorageCapacityClusterInformer(client versioned.ClusterInterface, resyncPeriod time.Duration, indexers cache.Indexers, tweakListOptions internalinterfaces.TweakListOptionsFunc) cache.SharedIndexInformer {
...
func (f *cSIStorageCapacityClusterInformer) defaultInformer(client versioned.Interface, resyncPeriod time.Duration) cache.SharedIndexInformer {
	return NewFilteredCSIStorageCapacityClusterInformer(client, f.namespace, resyncPeriod, cache.Indexers{
