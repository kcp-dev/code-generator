...
func (c *clusterTestTypesClusterClient) Cluster(clusterPath logicalcluster.Path) upstreamexamplegroupv1client.ClusterTestTypeInterface {
...
func (c *clusterTestTypesClient) ApplyScale(ctx context.Context, deploymentName string, applyConfiguration *examplev1.ClusterTestTypeApplyConfiguration, opts metav1.ApplyOptions) (*autoscalingv1.Scale, error) {
	if applyConfiguration == nil {
		return nil, fmt.Errorf("applyConfiguration provided to Apply must not be nil")
	}
	data, err := json.Marshal(applyConfiguration)
